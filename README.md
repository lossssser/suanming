...
- 《死者的顿足舞》：基于用户提供资料的结构化私用改编。
- 《牙仙子》：基于布罗克方块创作的 COC7th 模组《牙仙子》v1.0 做结构化私用改编；保留作者署名，仅供个人学习交流与非商业使用。

跑团不会在网页中公开原始 PDF 或完整原文。跑团使用同一个 D1 binding `DB` 和 Worker Secret `DEEPSEEK_API_KEY`。Worker 首次访问 `/trpg` 时会自动创建 `trpg_rooms` 表。
